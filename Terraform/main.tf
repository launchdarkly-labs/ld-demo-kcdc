...
resource "launchdarkly_project" "kcdc" {
  key  = "ld-demo-kcdc"
  name = "ld-demo-kcdc"
...
resource "launchdarkly_feature_flag" "qrcode" {
  project_key = launchdarkly_project.kcdc.key
  key         = "qrcode"
  name        = "QR Code"
...
output "LaunchDarkly_API_Key" {
  value = launchdarkly_project.kcdc.environments[0].api_key
  sensitive = true
}

output "LaunchDarkly_Client_Side_Key" {
  value = launchdarkly_project.kcdc.environments[0].client_side_id
  sensitive = true
}
